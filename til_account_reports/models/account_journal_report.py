...
class JournalReportCustomHandler(models.AbstractModel):
    ...
    def _query_journal(self, options):
        params = []
        queries = []
        report = self.env.ref('til_account_reports.journal_report')
    ...
    def _get_first_move_line(self, options, parent_key, line_key, values, is_unreconciled_payment):
        ...
        for column_group_key, column_group_options in report._split_options_per_column_group(options).items():
            ...
            columns.extend([
                {'name': '%s %s' % (values['account_code'], '' if values['partner_name'] else values['account_name']), 'name_right': values['partner_name'], 'class': 'o_account_report_line_ellipsis' + (' color-blue' if not_receivable_with_partner else ''), 'template': 'til_account_reports.cell_template_journal_audit_report', 'style': 'text-align:left;'},
    ...
    def _get_aml_line(self, options, parent_key, eval_dict, line_index, journal, is_unreconciled_payment):
        ...
        for column_group_key, column_group_options in report._split_options_per_column_group(options).items():
            ...
            if values['journal_type'] == 'bank':  # For additional lines still showing in the bank journal, make sure to use the partner on the account if available.
                ...
                account_name_col = {'name': account_name, 'class': 'o_account_report_line_ellipsis' + (' color-blue' if not_receivable_with_partner else ''), 'name_right': values.get('partner_name'), 'style': 'text-align:left;', 'template': 'til_account_reports.cell_template_journal_audit_report'}
            else:
