...
class AccountReport(models.Model):
    ...
    main_template = fields.Char(string="Main Template", required=True, default='til_account_reports.main_template')
    main_table_header_template = fields.Char(string="Main Table Header Template", required=True, default='til_account_reports.main_table_header')
    line_template = fields.Char(string="Line Template", required=True, default='til_account_reports.line_template')
    footnotes_template = fields.Char(string="Footnotes Template", required=True, default='til_account_reports.footnotes_template')
    search_template = fields.Char(string="Search Template", required=True, default='til_account_reports.search_template')
    ...
    def open_report_export_wizard(self, options):
        ...
        view_id = self.env.ref('til_account_reports.view_report_export_wizard').id
    ...
    def caret_option_open_general_ledger(self, options, params):
        ...
        gl_options = self.env.ref('til_account_reports.general_ledger_report')._get_options(options)
        gl_options['unfolded_lines'] = [account_line_id]

        action_vals = self.env['ir.actions.actions']._for_xml_id('til_account_reports.action_account_report_general_ledger')
        action_vals['params'] = {
            'options': gl_options,
            'ignore_session': 'read',
        }
    ...
    def _generate_accounts_coverage_report_xlsx_lines(self):
        ...
        accounts_in_coa = []
        if self.root_report_id and self.root_report_id == self.env.ref('til_account_reports.profit_and_loss'):
        ...
        elif self.root_report_id and self.root_report_id == self.env.ref('til_account_reports.balance_sheet'):
            accounts_in_coa = self.env["account.account"].search([
                *common_account_domain,
